    ...
    def run(
        ...
        def _execute_search():
            ...
            try:
                ...
                # Force is_available to True for SearXNG if it's somehow False
                if hasattr(self, 'is_available') and not self.is_available:
                    logger.warning(f"[BaseSearchEngine] {self.__class__.__name__} is_available was False, forcing to True before _get_previews")
