...
class WikipediaSearchEngine(BaseSearchEngine):
    ...
    def _get_previews(self, query: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Extract keywords if query is too long
            search_query = query
            if len(query) > 80:  # Wikipedia API often fails with very long queries
                if self.llm:
                    try:
                        prompt = f"Extract 3-5 main keywords from this search query for Wikipedia. Return ONLY the keywords separated by spaces: {query}"
                        response = self.llm.invoke(prompt)
                        if hasattr(response, "content"):
                            search_query = response.content.strip()
                        else:
                            search_query = str(response).strip()
                        logger.info(f"Optimized Wikipedia query: '{query}' -> '{search_query}'")
                    except Exception as e:
                        logger.warning(f"Failed to extract keywords with LLM: {e}")
                        search_query = query[:100]  # Fallback truncation
                else:
                    search_query = query[:100]  # Fallback truncation

            # Get search results (just titles)
            search_results = wikipedia.search(search_query, results=self.max_results)
