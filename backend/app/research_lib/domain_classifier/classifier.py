...
# Try to import database models and session context, fallback to None if not available
try:
...
class DomainClassifier:
    ...
    def _get_domain_samples(
        ...
        """
        if not _DATABASE_AVAILABLE or not ResearchResource:
            return []
        resources = (
...
JSON Response:"""
    ...
    def classify_domain(
        ...
        """
        if not _DATABASE_AVAILABLE or not get_user_db_session:
            return None
        try:
