...
class SourceBasedSearchStrategy(BaseSearchStrategy):
    ...
    def analyze_topic(self, query: str, initial_search_queries: Optional[List[str]] = None, **kwargs) -> Dict:
        """
        Analyze a topic using source-based search strategy.
        
        Args:
            query: The research query
            initial_search_queries: Optional list of pre-decomposed search queries to use in first iteration
        ...
        try:
            ...
            for iteration in range(1, iterations_to_run + 1):
                ...
                # For first iteration, use initial query or pre-decomposed queries
                if iteration == 1:
                    # If initial_search_queries provided, use them instead of generating
                    if initial_search_queries and len(initial_search_queries) > 0:
                        logger.info(f"Using {len(initial_search_queries)} pre-decomposed search queries for first iteration")
                        all_questions = initial_search_queries
                        # Store them in questions_by_iteration for tracking
                ...
                for i, future in enumerate(
                    ...
                    links_preview = []
                    for r in (search_results[:5] if isinstance(search_results, list) else []):
                        url = (r.get("link") or r.get("url") or None)
                        title = r.get("title") or ""
                        if url:
                            links_preview.append({"title": title, "url": url})
                    self._update_progress(
            ...
            if self.use_cross_engine_filter:
                ...
                links_preview_final = []
                for r in (final_filtered_results[:5] if isinstance(final_filtered_results, list) else []):
                    url = (r.get("link") or r.get("url") or None)
                    title = r.get("title") or ""
                    if url:
                        links_preview_final.append({"title": title, "url": url})
                self._update_progress(
                    ...
                    iteration_progress_base + 85,
                    {
                        ...
                        "iteration": iteration,
                        "links_count": len(self.all_links_of_system),
                        "links_preview": links_preview_final,
                    },
                )
            else:
