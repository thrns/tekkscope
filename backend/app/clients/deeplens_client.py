...
from app.research_lib.api.research_functions import detailed_research
...
from app.research_lib.utilities.prompt_enhancer import enhance_deeplens_prompt
...
def create_settings_snapshot(request: DeepLensRequest) -> dict:
...
async def get_detailed_research(request: DeepLensRequest, user_id: str, api_key_id: str | None = None) -> dict:
    if user_id and api_key_id:
        await check_rate_limit(api_key_id, user_id)
    elif user_id:
         env_api_key = os.getenv("CURRENT_API_KEY_ID")
         if env_api_key:
             await check_rate_limit(env_api_key, user_id)
    ...
    try:
        ...
        preprocessed = {
            ...
            "synthesis_prompt": enhance_deeplens_prompt(request.query),
    ...
    try:
        ...
        if input_tokens + output_tokens != total_tokens:
            output_tokens = total_tokens - input_tokens
        credits_used = compute_credits_used(input_tokens, output_tokens, "TEKKSCOPE", "deeplens")
        provider = "GEMINI"
        model = os.getenv("TEKK_LLM_MODEL", "gemini-2.0-flash")
        research_id = serial.get("research_id") or str(uuid.uuid4())
...
async def stream_detailed_research(query: str, client_id: str, user_id: str, api_key_id: str | None = None) -> None:
    ...
    settings_snapshot = create_settings_snapshot(DeepLensRequest(query=query))
    ...
    try:
        ...
        try:
            ...
            preprocessed = {
                ...
                "synthesis_prompt": enhance_deeplens_prompt(query),
        ...
        await manager.send_to_client(client_id, {"type": "thinking", "content": {"phase": "calling_detailed_research", "iterations": 0, "query": query}})
        try:
            ...
            return
        await manager.send_to_client(client_id, {"type": "thinking", "content": {"phase": "compiling_report", "iterations": 1, "query": query}})
        serial = to_serializable(result)
        try:
            summary_text = serial.get("summary") or ""
        ...
        def pick_text(d):
            if isinstance(d, dict):
                for k in ["report","summary","answer","content","text"]:
                    v = d.get(k)
